...
> `-SL200`) opened **26 contracts**; `GF-QQQ-IC-Canary` opened **1 contract** and its exit
> fired 28 minutes later, first of the seven. The ledger agrees independently — post-ingest
> `data/trades.csv` rows with `open_date >= 2026-09-03` carry quantity 26 / 1.
> Evidence: `data/captures/2026-09-07-catchup/03-layer2-2026-09-07.md`.
>
> **STILL OPEN — both sides.** On the one eligible fill day (**2026-09-04**) every arm
> opened the **put side only**. Read out of the bot Log, not inferred: `GF-ScannerB-CallSpread`
> passed all four gates every minute of the 1:30–2:00pm window, built the order
> (`QQQ Short Call Spread -719 / +721`), and was rejected by its own credit filter —
> **"Filtered: Mid price is $0.07."** Not the limits (arms read 2/2 first-hand), not the
> 26-ct edit, not a dead scanner. **The consequence is a one-sided 26-contract position,
> $4,940 at risk, carried naked to expiry.** It expired worthless and paid; the exposure
> shape was not the intended condor. A both-sides day at 26 contracts has not yet been
> observed and this line stays until one is.
>
> **2026-09-03 produced no fills at all** — QQQ was up more than +0.75% since the previous
> close, so the range filter's upper bound rejected the day on both scanners. Legitimate;
> no finding.
>
> Sample: seven positions, one day, one side. Per `CLAUDE.md` §4 nothing here supports a
> live-capital or growth decision.
> ⛔ Note for whoever writes this into the loop: **`STATUS.md` is generated by `scripts/report.py`
> and says "Do not edit by hand"** — a banner pasted there is erased on the next `daily.sh`.
> This block is the durable carrier; the same text is on PR-14…PR-20 as `LAYER 2 OUTSTANDING`.

## 2026-09-07 — Catch-up close: 09-01…09-04 ingested, Layer 2 read, zero config drift

Source: Cowork + Claude in Chrome, 12:24–13:00 ET, **read-only on OA — no edits made, none
queued**. Evidence bundle `data/captures/2026-09-07-catchup/`. Labor Day, markets closed.

- **Four days ingested in one run.** `scripts/close.sh 2026-09-04`, all stages exit 0, no
  override flag. The OA export is full history and `build_ledger.py` rebuilds from
  `newest_raw()` alone (lines 76–79), so one run folds 09-01…09-04 — separate per-day runs
  were neither needed nor made. **STATUS.md $4,557 / 205 legs → $8,160 / 246 legs.** Both
  sides derived and matched: the export carries exactly 41 rows with `closeDate` in
  09-01…09-04 and `openDate >= LEDGER_START` summing to **+$3,603**; 205+41=246 and
  4,557+3,603=8,160.
- **Layer 2: quantity verified, both-sides not.** See the banner above and
  `03-layer2-2026-09-07.md`.
- **Zero config drift.** All 44 bots byte-identical to the 2026-09-02 POST capture on both
  instruments — `allbots` (allocation, group, tags, limits, account) and the `i.sticon`
  AUTOS/EXITS titles — compared per row by signature, not by eye. AUTOS 18/44, EXITS 16/44,
  unchanged. `01-drift-verdict-2026-09-07.md`.
- **`GF-QQQ-IC-Ride-Delta` still AUTOS OFF**, confirmed by screenshot. Still tagless, so
  `R-2026-09-02-PR23-TAG-NOT-APPLIED` stands.

### ⛔ TWO THINGS FOR ANDY

1. **`R-2026-09-03-3DTE-POSITION-SIZE-CORRECTION` closes the opposite way.** Its premise was
   a 09-02 read of `a5.bots.bot.inputs` returning `1 contract`. Opened fresh 2026-09-07, the
   bot's **Bot Inputs panel** and the **automations' own `inputs` arrays** both read
   **`Position Size = 26% of net liquid`**, and all 22 Open-Position `amount` nodes across the
   `xDTE $140-$350 +10` automation are `type:"input"` references to that bot input — **zero
   literals, no automation-level size override anywhere**, which is the follow-up the ruling
   asked for. The `1 contract` in `a5.bots.bot.inputs` looks like the *template default*, not
   the effective value. If so the 08-31 record was right, the 08-31 `bots_meta` SIZING EPOCH
   note is **not** spurious, and the $10K→$5K revert **did** halve per-position dollar size —
   the opposite of what the ruling concludes. Sizing is always gated; **not applied, not
   edited.** The R-4 decision (allocation $5,000) is unaffected — it stands on PR-07's own
   "$5K, deliberately small" line, and OA reads $5,000 today.
   Detail: `04-followups-2026-09-07.md`.
2. **A catch-up close records `capture: ABSENT` even when a capture is supplied.**
   `close_manifest.py:352` looks for `data/captures/<day>-roster`; `capture_bundle.py` names
   the bundle from the capture's own date. Same-day closes coincide, catch-up closes never do
   — here the bundle landed in `data/captures/2026-09-07-roster` and the 2026-09-04 manifest
   still reads ABSENT. Latent since the WRAP; this is the first catch-up close with a capture
   to expose it. A fix changes the daily-loop contract, so it is gated, not made.

## 2026-09-02 — Sizing executed on OA: GF family 1 ct → 26 ct, Canary detached, 3DTE reverted

Source: Cowork + Claude in Chrome, 20:59–22:28 ET (OA page clock; the container clock read
2026-09-03 UTC). Evidence bundle `data/captures/2026-09-02-gf-sizing/`. Authority: the seven
rulings signed by Andy 2026-09-02, now in `docs/RULINGS.md`.

- **The six live greenfield arms are at 26 contracts, ~$5,018 risk per position.** One edit per
  shared library scanner — `amount.quantity` 1 → 26 — not seven per-bot edits, so drift is
  structurally impossible. Verified after a hard reload from `a5.bots.acedit.routine`, and again
  from `GF-QQQ-IC-Ride`'s own page. New CONFIG HASHes: ScannerA
  `2c4a96c5e2e3c850…`, ScannerB `a1a48af1caebc359…`. A full leaf-by-leaf diff of the routine
  before/after shows **four changed leaves, all of them the same quantity field** — strikes,
  wings, delta, gates, exits bindings and tags are byte-identical.
- **The gate passed before anything was edited.** `GF-ScannerA/B` were attached to exactly the 8
  `GF-QQQ-IC-*` arms and nothing else; both pre-hashes byte-matched the ruling's stamps; both
  read quantity 1. Had any non-GF bot been attached, the shared lever would have been abandoned.
- **`GF-QQQ-IC-Canary` stays at 1 contract and is off the shared pair.** Bot-local
  `Canary-Scan-Put` / `Canary-Scan-Call` were created by Duplicate → "Yes, create a copy",
  proven field-identical to the shared originals (`inputs` and `root` equal ignoring node ids),
  then the two shared scanners were removed **from this bot only** — dialog read verbatim
  "Remove scanner?". The library still holds its original four automations; ScannerA/B now read
  **7 bots** and their hashes are byte-identical to before the detach.
- **Allocations**: six arms $2,500 → $10,000; `3DTE $140-$350` $10,000 → **$5,000** (R-4 revert).
  Allocation is a container, not position size — the size lever was the scanner.
- **Groups and tags**: three `-ARCHIVED-` clones moved into `Archive`; group `IC` renamed
  **`GF-Family`** (8 members, unchanged); **10 `PR-NN` tags added**, so `pr 01`…`pr 20` now each
  appear exactly once, on the right bot.
- **Fleet control**: `/bots` captured before and after with the same instrument. **44 bots,
  AUTOS ON 18/44, EXITS ON 16/44 — identical in both.** Footer identical. The only diffs are the
  intended allocations, the derived percentages, and market marks on two bots holding open
  positions.

### ⛔ TWO THINGS ANDY MUST RULE ON — neither was executed

1. **The GF position limits were NOT set to 1/1.** `R-2026-09-02-POSITION-LIMITS` says "set GF
   arms 1/1". Its execution guard assumed they already were — citing the 08-31 capture — but
   `06-ride-delta-scanner-diff-2026-08-31.md` §5 says the opposite in writing ("DAILY POSITIONS
   2 per day, POSITION LIMIT 2 at once"); the bot that capture recorded at 1/1 is the *mirror*
   `3DTE $140-$350`. All seven GF arms read **2/2** first-hand on 2026-09-02. **A GF condor is
   two OA positions** (ScannerA opens the put side, ScannerB the call side, same day), so a
   counter of 1/day opens one side and leaves a naked position — the damage the 08-31 forensics
   documented. PR-14…PR-20 already carry a signed line saying so: *"GO-LIVE ORDERING
   `R-2026-08-09-S2-R5-LIMITS` — condor-aware: arms stay 2/2."* Sizing and OA bot behaviour are
   always gated, so the edit was withheld and escalated in-session.
   Register: `R-2026-09-02-GF-LIMITS-WITHHELD`.
2. **`3DTE $140-$350`'s "26% of net liquid" is not what OA reads today.** Its Bot Input
   POSITION SIZE reads `{"text":"1 contract","type":"quantity","quantity":1}` — a fixed 1
   contract — read first-hand from `a5.bots.bot.inputs` after a hard reload. The 08-31 record and
   R-4's rationale both state the percentage. The revert is applied either way; the *consequence*
   claimed on 08-31 (and repeated in `bots_meta.csv`) should be re-checked before it is quoted
   again. Its automations were not opened, so an automation-level override is not excluded.

Also unresolved, smaller: the **PR-23 tag was not applied** to `GF-QQQ-IC-Ride-Delta` — that bot
carries no tags at all and its tag widget refused input across three attempts
(`R-2026-09-02-PR23-TAG-NOT-APPLIED`). PR-23 is itself DRAFT/unsigned and the bot is AUTOS OFF.
...
## 2026-08-12 — 8th greenfield arm added
...
*A SCOPED follow-on to S2, same day. Ten bots switched on, PR-02 and PR-04 signed and live, four
findings, five Andy rulings. Full record: `session-log.md` 2026-08-09 S2b; evidence
`data/captures/2026-08-09-s2b/` (2 text files + 30 screenshots). No git.
⛔ **This block SUPERSEDES the "7 of 43" ON-count in the S2 block below**, which is correct as of
its own timestamp and historical thereafter.*

- ⭐ **AUTOMATIONS ON = 17 of 43 · EXIT OPTIONS ON = 15 of 43 · footer `43 active bots • 7 left in
  your plan`.** The ON set is EXACTLY: the seven GF arms · **PR-01** `IC-SPX-FastPT25-S2` ·
  **PR-02** `IC-SPX-FastPT25-S2-130PM` · **PR-04** `QQQ-IC-0DTE-Fortress-NoPT50` · **PR-05**
  `DIR-SPX-PutVIX22-SL75` · **PR-06** `DIR-SPX-CallVIXdrop` · **PR-07** `3DTE $140-$350` ·
  **PR-08** `Nigiri-Paper-v1` · **PR-10** `Friday 14 DTE Broken Wing IB (B-70)` · **PR-11**
  `Trendy-Paper-v1` · **PR-12** `60min-ORB-10W-Paper-v1` — **and nothing else.** Verified on the
  second surface (`/bots` `i.sticon[title]`), one page read.
- ⭐ **POSITION LIMITS ARE NOW AT THEIR RULED VALUES ON EVERY ON BOT.** Seven GF arms 2/2
  (untouched). PR-01 **10/10 → 2/2** · PR-02 **10/10 → 2/2** · PR-07 **1/5 → 1/1** · PR-08
  **10/10 → 1/1** · PR-10 **3/10 → 1/1** · PR-11 **2/2 → 1/1**. PR-05, PR-06, PR-12 already
  stored 1/1 and PR-04 already stored 2/2 — **recorded and skipped, not force-edited**; nothing
  forked, nothing to revert. **Allocations untouched on every bot.** Each edit proven by a hard
  reload + fresh model read + post-reload screenshot; A-27a re-read per bot on two surfaces with
  a hard abort, 0 aborts.
- ⭐ **PR-02 AND PR-04 ARE SIGNED** (pre-reg §4). Template V1 was the last block on both and Andy
  created both by hand: `PR-02 Tfw5TkkCRF1717862960217463041` · `PR-04
  Tfw5TkkCRF1717862960769161432`, My Templates **10 → 12**, and — the check that actually proves
  attachment — **each clone's own `tid` now resolves** (both were `null`). PR02-R1 was already
  ruled/discharged 2026-08-08. ⛔ Group-B's only remaining placeholder is **PR-03**, unsigned by
  ruling. **Signature count 16 → 18.**
- ⭐ **THE EXIT-OPTION-FREE CONTROLS HELD.** PR-01 and PR-02 went ON with **EXIT OPTIONS OFF**
  (`disableExits 1`), which is CORRECT and was enforced by an **inverted guard** that would have
  aborted on `true`. Their Monday check is the INVERTED one. Never "fix" them.
- ⛔ **STEP 6 IS NOT SATISFIED FOR ANY OF THE 17.** Market closed 2026-08-09; the test-fire
  (S2-R2) yields no Trades list and was NOT fired. All 17 are ⬜ NOT EVALUABLE, **due
  2026-08-10** from the first new position. The ruled limits ARE the first-position control.
- ⛔ **GATE A12 STILL OWED.** `data/bots_config_v2.csv` carries a dated **staleness banner
  (comment lines only — no data row touched)** naming all 17 stale bot rows. ⭐ Checked, not
  assumed: **nothing machine-consumes the stale columns** — `a_series.py`'s `load_config_a7()`
  reads only `shared_automation` rows and still returns its 3; Tier C is already SKIPPED BY NAME;
  `daily_brief.py` is already config-blind. The bookmarklet + Export Data sweep is Andy's hand and
  is **not** a Monday blocker.

### FOUR FINDINGS — all first-hand
| # | Finding | Disposition |
|---|---|---|
| **S2b-R1** | The S2b launch prompt's **PR-01 bot id was wrong** (it flagged the doubt itself). `…3017861977616287731` is **PR-02**; PR-01 is **`BOTfw5TkkCRF4417860821948715488`**. Three independent sources agree, incl. PR-01's own Notes panel. | ✅ Caught BEFORE any edit. **No edit on the wrong bot.** Recorded. |
| **S2b-R2** | The three archive-queued originals are **not all dated 08-08** — `IC-SPX-FastPT25-S2-ARCHIVED-**2026-08-07**` carries the other label. | Recorded. **Hand item 4 is verified by ID + footer 43→40, never by name.** No doc edit. |
| **S2b-R3** | ⛔ **PR-04's EXIT OPTIONS read OFF** — and PR-04 is NOT an Exit-Option-free control. Its spec needs the 15:50 Expiration exit (an Exit Option) to fire; with the switch off the 15:52 backstop closes everything **by construction** and Step 6 is meaningless on day one. Never armed because PR-04 sits outside the nine S2's Step 3 swept. | ✅ **RULED: arm, then switch on.** Andy then **challenged his own ruling** ("NoPT50 → no exit options?"); the flip was HELD and answered first-hand: `profits ""` on both Open Position actions, `expdays 0.01` the only live rung, **both config hashes re-computed live and byte-identical**. Andy re-confirmed "Go". **ARMED + ON.** |
| **S2b-R4** | 📝 `expdays = 0.01` (= 14.4 min ⇒ ~15:45–15:46) vs the folder's **"15:50 exit"** label on ≥3 surfaces. ⛔ The figure is **arithmetic on the stored value, NOT an observation of OA's semantics.** | ✅ **RULED: record only, correct the docs after Monday.** No doc edit. **Monday check owed:** the OBSERVED close time. Ordering PR-04 depends on still holds. |

### ⛔ CARRIED, NOT CLOSED
**S2-R3** (`a_series --validate` = 1 true-positive divergence) — ✅ **DEFERRED by Andy at S2b**,
not ruled. Prints at every close-out; A1–A9 green, nothing blocked. Session recommendation on
file: option A, re-baseline `RECORDED_NOTE_LEN` — a `scripts/` edit, no OA surface, **Claude Code
lane**.
> ⛔ **CORRECTED 2026-08-09 (orchestrator, same day, evidence-backed — the line above stands as
> the S2b worker's honest record):** S2-R3 was in fact **RULED BY ANDY — OPTION A — in the
> orchestrator chat BEFORE S2b opened**, and applied there: `scripts/a_series.py`
> `RECORDED_NOTE_LEN` re-baselined to the signed §9 lengths (build-time values kept in a dated
...
only) · `data/captures/2026-08-09-s2b/` (32 files) · `docs/state.md` (this block) ·
`docs/session-log.md`. All device-hash-verified. **Uncommitted — Andy commits.**
...
> "Unused"), all 9 Bot Templates (the 7 `GF-QQQ-IC-*` + `QQQ-IC-0DTE-Fortress` + `TEST HedgeA-S1`), **[CORRECTED 2026-08-08, sweep FS-2: TEN live — the +1 is the IC-SPX-FastPT25-S2 Template V1 deliberately created 2026-08-07 (tid Tfw5TkkCRF2317861409017023081, vdate 22:15:01Z), after this count was written. Accounted for, applied at the orchestrator close-out.]**
...
## ⭐ READ-ONLY FLEET AUDIT SWEEP — 2026-08-08 (post-PR-04). **43·7 CONFIRMED. ZERO OA WRITES. FIVE ITEMS GATED, NONE APPLIED.**

> **The fleet, as it actually is, read fresh today and diffed against the record.** Nothing was
> fixed, toggled, renamed, saved or archived. Disk writes: `data/captures/2026-08-08-audit/`
> (7 files) + the session-log entry + this block. No git.
>
> **ROSTER — `/bots` footer verbatim: "43 active bots · 7 left in your plan".** = the 2026-08-07
> restore's 41 + the PR-02 clone + the PR-04 clone. Gate A0 **BRANCH 1**. No duplicate production
> names. Three `-ARCHIVED-` names on `/bots` is the KNOWN state (archives queued for Andy, gate A5).
>
> **⭐ A-01c IS CLEAN — the relational asserts cannot see this, so it was done by hand.**
> **43/43 bot IDs match** (41 preserved · 0 lost · 2 new clones matching `bots_config_v2.csv` ·
> the 2 renames carry the **SAME id**, renamed not re-created). **32/32 recorded automation rids
> match**, the three shared objects read **separately from all seven arms** so attachment is
> proven per-arm, not assumed. 13 further rids recorded as **FIRST BASELINES** (pilot 3, and the
> three archived originals) — they prove nothing about the past and are labelled so.
> **A-01 branch 3c: NOT PRESENT.**
>
> **⭐ TOGGLE STATE, one batched `i.sticon[title]` read of all 43 rows:**
> **AUTOMATIONS ON = 0 of 43. EXIT OPTIONS ON = 7 of 43 — exactly the seven greenfield arms.**
> **Zero drift** against 2026-08-07 on every surviving bot. S0a's seven `disableExits` flips now
> persist across a THIRD session boundary. ⛔ **A-27(a)'s carry-forward is NOT retired by this:**
> toggle state is re-read first-hand immediately before Step 7, on every bot, never inherited —
> including from this sweep.
>
> **⭐ THE AUTOMATION LIBRARY IS AT `/bots/automations`** (not `/automations` — PR04-R2's 404
> was a wrong path, not a broken page; its `sharing = 0` reasoning stands). Enumerated: **1 folder,
> 4 objects, bot-counts 7/7/7/2 as expected**, attached-bot lists read via `showBots`.
> **A7 RUN LIVE AGAINST OA** (formula validated first against ScannerB's recorded baseline, per
> A-15): all **3 of 3** recorded hashes byte-identical — ScannerA `3308ce8b…` v9 (the ADOPTED
> A7-DRIFT-1 baseline, no new drift), ScannerB `bb4ba866…` v2, Backstop `116069bd…` v1. A second
> independent evidence path agreeing with the script's capture-file A7.
> **A-13:** `Defang-Mon-S2-StrikeTouch` = rid `RTfw5TkkCRF3317787955826108344`, v5, sha256
> `291e05ad09c2f6f801a7dcab0a121d503525fb6c015e27763cca3effa20155b6`. ⛔ **FIRST baseline, not a
> re-verification. A7 remains 3/4 VERIFIED + 1 FIRST BASELINE — never 4/4.** The
> `bots_config_v2.csv` row was **NOT written**; it is GATED and drafted for Andy.
>
> **A-SERIES via `scripts/a_series.py` (A-26), script unedited.** `--validate` reproduced the
> reference EXACTLY. **A1 21/21 · A2 7/7 · A3 7/7 · A4 MOOT · A4b NOT-RUNNABLE · A5 NOT-RUN ·
> A6 NOT-RUNNABLE · A7 3/3 · A8 7/7 · A9 7/7 · §8.2 6/6+2/2+13/13 · FAMILY GREEN.**
> ⛔ **A4b did NOT fire and that is NOT a pass** — today's O4 fix made it fire against the
> SELF-TEST fixture; against the real repo `LEDGER_START = 2099-01-01` leaves zero rows, so it is
> NOT-RUNNABLE. It becomes a live detector on the first post-cutover trading day, not before.
>
> **COUNTS.** Bot Archive **1 of 1** (`QQQ-IC-0DTE-Fortress-ARCHIVED-2026-08-03`) at
> **`/settings/archive`** — a path no project document names (`/bots/archive` 404s). Templates
> **10** live vs **9** recorded above; the +1 is `IC-SPX-FastPT25-S2` (tid
> `Tfw5TkkCRF2317861409017023081`, vdate 2026-08-07T22:15Z — created after that count was taken,
> same day). PR-02 and PR-04 still have **NO** template (`tid` ABSENT).
> Account settings: **`itmpaper` = `itmlive` = `market`** — S0a's write persists, no `auto`
> rollback (A-01 sub-state 3d watch item, clear).
>
> **SPOT-VERIFIES.** PR-02 clone **seed 50000 / $50K** = PR02-R1 as ruled (original still $30K).
> Both new clones **AUTOMATIONS OFF + EXIT OPTIONS OFF** on two independent surfaces each.
> **Pilot `QQQ-IC-0DTE-Fortress` `disableExits` = 1 → EXIT OPTIONS OFF**, against its own
> 2026-08-04 capture's `ON` — **S0b-1, now a THIRD dated first-hand observation. RULED DO NOT FIX
> (A-27(a)). Read, recorded, NOT fixed.** PR-03 stays UNSIGNABLE, as expected.
>
> **⛔ FIVE ITEMS GATED — NONE APPLIED. Full text: `session-log.md`, this date.**
> **FS-1** (LOW) `scripts/a_series.py:507` prints `itmlive=auto`; live and this page say `market`.
> Changes no verdict (A5 is NOT-RUN) but prints into every close-out. Claude Code's lane.
> **FS-2** (INFO) this page's "all 9 Bot Templates" is stale at 10 — accounted for, not corrected.
> **FS-3** (INFO) the Library path, a doc-correction candidate against PR04-R2 and the pack.
> **FS-4** (LOW) `exitrate` ABSENT from `a5.bots.bot` on **all seven** non-greenfield bots opened,
> while all seven GF arms carry a stored `1` — the same not-stored condition A2-EXITRATE-1 found.
> No CSV column holds it; adding one is a **schema decision**, same disposition as before.
> **FS-5** (INFO) the sweep prompt said "8 greenfield bots"; the record carries **seven**.
> Plus a documentation gap, not a mismatch: **no project document names `/settings/archive`**.
>
> **⛔ ATTESTATION (A-18): NONE of the nine leave-in-place bots was opened, edited or
> toggle-touched.** Exactly fourteen bot pages were opened — the 7 GF arms, the three clones,
> their three archived originals, and the pilot. **STEP 2c REMAINS UNSPENT.**
> **⛔ Unchanged by this sweep:** the nine still have no per-bot capture (9 of CA-3's 11-bot
> worklist), A-07 disposition ⬜ NOT EVALUABLE; and the five open positions (QQQ long call 4,
> Tasty Condor 1, account RISK **$14,118** — identical to 08-07) were read from the LIST VIEW
> only, so A-10's first-hand re-read is untouched.
>
