...
> ## ⚠️ UNVERIFIED-L2 — CARRY THIS AT THE TOP OF EVERY BRIEF UNTIL CLOSED (opened 2026-09-02)
>
> **The first new position on each of `GF-QQQ-IC-Ride` / `-PT50` / `-Trail` / `-Touch0` /
> `-SL100` / `-SL200` must show quantity 26 in the TRADES LIST.**
> **`GF-QQQ-IC-Canary` must show quantity 1 and must fill its 5% profit target.**
> **Both sides — put spread AND call spread — must still open on every arm.**
> **The Exit Options panel is NEVER evidence** (`CLAUDE.md` §5, `oa-platform-reference.md` §0.3).
>
> Layer 1 is complete and recorded (`data/captures/2026-09-02-gf-sizing/`). Layer 2 is UNRUN
> because 2026-09-02 was already closed (21:00 ET) when the edits landed. First eligible
